...
def AddItemInMenu(items):
    for item in items:
        if item['isDir'] == True:
            mode = '1' # Mode par défaut pour les dossiers
            if 'mode' in item and item['mode'] == 'load_specific_slug':
                mode = '3' # Utiliser le mode '3' pour le slug spécifique
            entry_url = sys.argv[0] + "?url=" + quote_plus(item['url']) + "&mode=" + mode + "&filters=" + quote(simplejson.dumps(item['filtres']))
            AddFolder(item, entry_url=entry_url)
        else:
...
def AddFolder(show, entry_url=None):
    strTitle = show['title']
    strURL = show['url']
    strImage =show['image']
